...
export default {
  ...
  data() {
    return {
      uploadUrl: process.env.VUE_APP_BASE_API + '/api/v1/uploadFile',
      list: null,
      total: 0,
      listLoading: true,
      listQuery: {
    ...
    createData() {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData.password = new MD5().update(tempData.password).digest('hex')
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          if (tempData.newPassword !== '') {
            tempData.password = new MD5().update(tempData.newPassword).digest('hex')
          }
